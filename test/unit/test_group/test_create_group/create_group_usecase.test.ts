...
describe("CreateGroupUseCase", () => {
    ...
    beforeEach(() => {
        ...
        useCase = new CreateGroupUseCase(groupRepo, userRepo, projectRepo);
    });
    ...
    it("should create a new group and return it with user names and project title", async () => {
        ...
        expect(result.newGroup.codSubj).toBe("MAT101");
        expect(result.newGroup.userIdList).toEqual(["f7c9d1e1-9d23-4f6e-94e1-8f45b50f2389", "b5c1d3e3-9c2b-46d1-97ee-c2d5d582a2d4", "e5f4g6h6-6i7j-4k1l-88hh-i2j3k4l5m6n7"]);
        expect(result.newGroup.yearSem).toBe(202501);
        expect(result.newGroup.projectId).toBe("470f1e7b-f645-4da6-9b59-2e7a9684b0cf");
        expect(result.newGroup.course).toBe("CIÊNCIAS DA COMPUTAÇÃO");
        expect(result.userNameList).toEqual(["Luke Skywalker", "Nuncio Perrela", "Peter Parker"]);
        expect(result.projectTitle).toBe("Desenvolvimento do site do Rokuzen");
    });
