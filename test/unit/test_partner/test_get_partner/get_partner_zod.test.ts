...
describe("GetPartnerSchema -Zod validation", () => {
    ...
    it("should throw (O name deve conter no mínimo 1 caracter) if name has less than 1 character", () => {
        try {
            parseBody(GetPartnerRequest, { name: "" })
        }
        catch (error: any) {
            expect(error.constructor.name).toBe("BadRequestException");
            expect(error.message).toBe("O name deve conter no mínimo 1 caracter")
            expect(error.statusCode).toBe(400)
        }
    });

    it("should throw (Você deve informar o id ou o name (exatamente um)) if id and name are provided", () => {
        try {
            parseBody(GetPartnerRequest, { id: "e9c7d747-9e8e-4d34-935e-473c2c16be83", name: "Real Madrid" })
        }
        catch (error: any) {
            expect(error.constructor.name).toBe("BadRequestException");
            expect(error.message).toBe("Você deve informar o id ou o name (exatamente um)")
            expect(error.statusCode).toBe(400)
        }
    });

    it("should throw (Você deve informar o id ou o name (exatamente um)) if neither id nor name are provided", () => {
        try {
