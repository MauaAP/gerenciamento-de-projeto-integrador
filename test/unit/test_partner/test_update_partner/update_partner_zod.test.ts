...
describe("UpdatePartnerSchema -Zod validation", () => {
    ...
    it("should throw (Setor não está entre os disponíveis) if sector is not among the available ones", () =>{
        try {
            parseBody(UpdatePartnerRequest, {id: "e9c7d747-9e8e-4d34-935e-473c2c16be83", sector: "INVALIDO"})
        }
        catch (error: any) {
            expect(error.constructor.name).toBe("BadRequestException");
            expect(error.message).toBe("Setor selecionado não está entre os disponíveis")
            expect(error.statusCode).toBe(400)
        }
    });

    it("should throw (Nome deve ter pelo menos um caracter) if name is an empty string", () =>{
        try {
            parseBody(UpdatePartnerRequest, {id: "e9c7d747-9e8e-4d34-935e-473c2c16be83", name: ""})
        }
        catch (error: any) {
            expect(error.constructor.name).toBe("BadRequestException");
            expect(error.message).toBe("Nome deve ter pelo menos um caracter")
            expect(error.statusCode).toBe(400)
        }
    });
});